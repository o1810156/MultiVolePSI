...
{
    void SepOpprfSender::oprfEval(span<const block> X, span<u8> out, span<const u8> add, u64 m, u64 numThreads)
    {
        auto n = X.size();
        auto nm = n * m;
    ...
    Proto SepOpprfSender::offline_send(u64 recverSize, PRNG &prng, u64 numThreads, Socket &chl, u64 myIdx)
    {
        MC_BEGIN(Proto, this, recverSize, &prng, numThreads, chl, myIdx);
        ...
        MC_AWAIT(mOprfSender.offline_send(recverSize, prng, chl, numThreads));

        MC_END();
    }

    Proto SepOpprfSender::online_send(u64 recverSize, span<const block> X, MatrixView<u8> val, PRNG &prng, u64 numThreads, Socket &chl)
    {
        MC_BEGIN(Proto, this, recverSize, X, val, &prng, numThreads, &chl,
                 ...
                 nm = u64{},
                 hashingSeed = block{},
                 type = volePSI::PaxosParam::Binary,

                 diffPtr = std::unique_ptr<u8[]>{},
                 diffU8 = span<u8>{});

        setTimePoint("SepOpprfSender::send begin");
        n = X.size();
        m = val.cols();
        nm = n * m;
        ...
        type = m % sizeof(block) ? volePSI::PaxosParam::Binary : volePSI::PaxosParam::GF128;
        mPaxos.init(n, 1 << 14, 3, 40, type, hashingSeed);

        if (mTimer)
            mOprfSender.setTimer(*mTimer);

        MC_AWAIT(mOprfSender.online_send(recverSize, prng, chl, numThreads));
        ...
        setTimePoint("SepOpprfSender::send eval");
        ...
        setTimePoint("SepOpprfSender::send paxos solve");

        MC_AWAIT(chl.send(coproto::copy(mP)));

        setTimePoint("SepOpprfSender::send end");

        MC_END();
    }

    void SepOpprfSender::eval(span<const block> val, MatrixView<u8> output, u64 numThreads)
    {
        auto m = output.cols();
        ...
        setTimePoint("SepOpprfSender::eval begin");
        if (m == sizeof(block))
        {
        ...
        setTimePoint("SepOpprfSender::eval paxos decode");

        // output +=  oprf(val)
        auto outU8 = span<u8>(output.data(), output.size());
        oprfEval(val, outU8, outU8, m, numThreads);

        setTimePoint("SepOpprfSender::eval end");
    }

    Proto SepOpprfReceiver::offline_receive(u64 setSize, PRNG &prng, u64 numThreads, Socket &chl, u64 myIdx)
    {
        MC_BEGIN(Proto, this, setSize, &prng, numThreads, chl, msg = std::string(), myIdx);
        ...
        MC_AWAIT(mOprfReceiver.offline_receive(setSize, prng, chl, numThreads));

        MC_END();
    }

    Proto SepOpprfReceiver::online_receive(
        u64 senderSize,
        ...
        Socket &chl)
    {
        MC_BEGIN(Proto, this, senderSize, values, outputs, &prng, numThreads, &chl,
                 ...
                 p = Matrix<u8>{});
        setTimePoint("SepOpprfReceiver::receive begin");
        ...
        MC_AWAIT(mOprfReceiver.online_receive(values, oprfOutput, prng, chl, numThreads));

        p.resize(paxos.size(), m, oc::AllocType::Uninitialized);
        MC_AWAIT(chl.recv(p));
        setTimePoint("SepOpprfReceiver::receive recv");
        ...
        setTimePoint("SepOpprfReceiver::receive end");
